...
export function JTandMagScreen() {
  ...
  const { items, isLoading, isFetchingMore, fetchMore, refetch } =
    useInfiniteContent(['jtandmag-page'], api.getJTandMag);
    ...
    <PageScreen
      ...
      renderItem={({ item }) => (
        <ContentCard
          ...
          onToggleFav={() => toggleFav(item.id, 'jtandmag')}
          onPress={() => navigation.navigate('ShowDetail', { id: item.id, type: 'jtandmag' })}
