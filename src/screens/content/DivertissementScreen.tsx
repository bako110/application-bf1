...
export function DivertissementScreen() {
  ...
  const { items, isLoading, isFetchingMore, fetchMore, refetch } =
    useInfiniteContent(['divertissement-page'], api.getDivertissement);
    ...
    <PageScreen
      ...
      renderItem={({ item }) => (
        <ContentCard
          ...
          onToggleFav={() => toggleFav(item.id, 'divertissement')}
          onPress={() => navigation.navigate('ShowDetail', { id: item.id, type: 'divertissement' })}
